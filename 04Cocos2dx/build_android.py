...
class AndroidBuilder(object):
    ...
    def do_build_apk(self, build_mode, no_apk, output_dir, custom_step_args, compile_obj):
        ...
        if not no_apk:
            ...
            if output_dir:
                #apk_out_time = time.strptime(apk_out_time,'%Y_%m_%d-%H_%M_%S')
                apk_out_time = time.strftime('%Y_%m_%d-%H_%M_%S',time.localtime(time.time()));
                ...
                if build_mode == "release":
                    signed_name = "%s-%s-%s-signed.apk" % (project_name, apk_out_time, build_mode)
                    apk_path = os.path.join(output_dir, signed_name)
                    if os.path.exists(apk_path):
                        os.remove(apk_path)
                    os.rename(os.path.join(output_dir, apk_name), apk_path)
                else:
                    debug_name = "%s-%s-%s.apk" % (project_name, apk_out_time, build_mode)
                    apk_path = os.path.join(output_dir, debug_name)
